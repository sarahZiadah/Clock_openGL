...
int main()
{
    ...
    Polygon boxF({ vec3(-hw, hh, hd), vec3(hw, hh, hd), vec3(hw, -hh, hd), vec3(-hw, -hh, hd) }, vec3(0.7f, 0.4f, 0.2f));
    Polygon boxB({ vec3(-hw, hh, -hd), vec3(hw, hh, -hd), vec3(hw, -hh, -hd), vec3(-hw, -hh, -hd) }, vec3(0.6f, 0.3f, 0.15f));
    Polygon boxL({ vec3(-hw, hh, -hd), vec3(-hw, hh, hd), vec3(-hw, -hh, hd), vec3(-hw, -hh, -hd) }, vec3(0.6f, 0.3f, 0.15f));
    Polygon boxR({ vec3(hw, hh, -hd), vec3(hw, hh, hd), vec3(hw, -hh, hd), vec3(hw, -hh, -hd) }, vec3(0.6f, 0.3f, 0.15f));
    Polygon boxT({ vec3(-hw, hh, -hd), vec3(hw, hh, -hd), vec3(hw, hh, hd), vec3(-hw, hh, hd) }, vec3(0.6f, 0.3f, 0.15f));
    Polygon boxD({ vec3(-hw, -hh, -hd), vec3(hw, -hh, -hd), vec3(hw, -hh, hd), vec3(-hw, -hh, hd) }, vec3(0.6f, 0.3f, 0.15f));


    float rodLength = 1.0f;
    vector<vec3> rodFront = { vec3(-0.05f, rodLength / 2, 0.05f), vec3(0.05f, rodLength / 2, 0.05f), vec3(0.05f, -rodLength / 2, 0.05f), vec3(-0.05f, -rodLength / 2, 0.05f) };
    Polygon rodF(rodFront, vec3(0.5f, 0.2f, 0.1f));
    vector<vec3> rodBack = { vec3(-0.05f, rodLength / 2, -0.05f), vec3(0.05f, rodLength / 2, -0.05f), vec3(0.05f, -rodLength / 2, -0.05f), vec3(-0.05f, -rodLength / 2, -0.05f) };
    Polygon rodB(rodBack, vec3(0.5f, 0.3f, 0.15f));
    vector<vec3> rodLeft = { vec3(-0.05f, rodLength / 2, -0.05f), vec3(-0.05f, rodLength / 2, 0.05f), vec3(-0.05f, -rodLength / 2, 0.05f), vec3(-0.05f, -rodLength / 2, -0.05f) };
    Polygon rodL(rodLeft, vec3(0.5f, 0.3f, 0.15f));
    vector<vec3> rodRight = { vec3(0.05f, rodLength / 2, -0.05f), vec3(0.05f, rodLength / 2, 0.05f), vec3(0.05f, -rodLength / 2, 0.05f), vec3(0.05f, -rodLength / 2, -0.05f) };
    Polygon rodR(rodRight, vec3(0.6f, 0.3f, 0.15f));
    vector<vec3> rodTop = { vec3(-0.05f, rodLength / 2, -0.05f), vec3(0.05f, rodLength / 2, -0.05f), vec3(0.05f, rodLength / 2, 0.05f), vec3(-0.05f, rodLength / 2, 0.05f) };
    Polygon rodT(rodTop, vec3(0.6f, 0.3f, 0.15f));
    vector<vec3> rodBottom = { vec3(-0.05f, -rodLength / 2, -0.05f), vec3(0.05f, -rodLength / 2, -0.05f), vec3(0.05f, -rodLength / 2, 0.05f), vec3(-0.05f, -rodLength / 2, 0.05f) };
    Polygon rodD(rodBottom, vec3(0.6f, 0.3f, 0.15f));


    Polygon clockFace = createCircle(0.8f, 60, vec3(1.0f, 1.0f, 1.0f), hd + 0.01f);
    mat4 clockFaceTransform = translate(mat4(1.0f), vec3(0.0f, hh - 0.9f, 0.0f));
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        pendulumTransform = translate(pendulumTransform, vec3(0.0f, -hh, 0.0f)); // عند أسفل الصندوق
